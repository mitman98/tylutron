...
class LutronConnection(threading.Thread):
  ...
  def _do_login_locked(self):
    ...
    #self._send_locked("#MONITORING,17,2")
    #self._send_locked("#MONITORING,17,3")
    #self._send_locked("#MONITORING,17,4")
    #self._send_locked("#MONITORING,17,5")
    ...
    #self._send_locked("?MONITORING,17,4")
    #self._send_locked("?MONITORING,17,5")
    #self._send_locked("?MONITORING,17,8")
    #self._send_locked("?MONITORING,17,11")
    #self._send_locked("?MONITORING,17,12")

    _LOGGER.info("Set up monitoring")
            ...
            if t is not None:
                ...
                if line:
                    try:
                        ...
                        _LOGGER.debug("Raw line from telnet: %r", decoded)
                        if decoded and decoded.startswith('~'):  # Only process response lines
                            self._recv_cb(decoded)
                    except UnicodeDecodeError:
                        _LOGGER.warning("Failed to decode line: %r", line)
            else:
                raise EOFError('Telnet object already torn down')
        except _EXPECTED_NETWORK_EXCEPTIONS as e:
            _LOGGER.exception("Network exception in main loop: %s", str(e))
            with self._lock:
                self._disconnect_locked()
            time.sleep(5)  # Wait before reconnecting
        except Exception as e:
            _LOGGER.exception("Unexpected error in main loop: %s", str(e))
            with self._lock:
                self._disconnect_locked()
            time.sleep(5)  # Wait before reconnecting

    _LOGGER.info("Main loop exiting")
...
class Lutron(object):
  ...
  def _recv(self, line):
    ...
    # Only handle query response messages
    ...
    try:
        ...
        _LOGGER.debug(
            "Processing response - type: %s, id: %d, args: %s",
            cmd_type, integration_id, args
        )
        
        if cmd_type not in self._ids:
            _LOGGER.info("Unknown command type %s in response: %s", cmd_type, line)
            return
            
        ids = self._ids[cmd_type]
        if integration_id not in ids:
            _LOGGER.warning("Unknown integration ID %d in response: %s", integration_id, line)
            return
        ...
        if not handled:
            _LOGGER.warning("Object %s did not handle update: %s", obj.name, args)
            
    except Exception as e:
        _LOGGER.exception("Error processing response line: %s", line)
...
class ThermostatCallStatus(Enum):
    ...
    HEAT_STAGE_1 = 1
    HEAT_STAGE_1_2 = 2
    NONE_LAST_COOL = 5  # Add if supported
    COOL_STAGE_1 = 6    # Add if supported
    COOL_STAGE_2 = 7    # Add if supported
